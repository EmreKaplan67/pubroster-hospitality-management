...
/** Generate roster PDF and return as Buffer for email attachment or download */
export function generateRosterPdfBuffer(
  staff: Staff[],
  shifts: Shift[],
  weekStart: string
): Buffer {
  ...
  const headers = ["Staff", ...DAYS.map((_, i) => formatDayHeader(weekStart, i))];
